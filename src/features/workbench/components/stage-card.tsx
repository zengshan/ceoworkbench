...
export function StageCard({ card, mode, focusState, selected, onSelect }: StageCardProps) {
  const tone = card.tone ?? 'paper';
  const compact = mode === 'compressed';
  const supporting = mode === 'supporting';
  const isBackground = focusState === 'background';
  ...
  return (
    <button
      ...
      className={clsx(
        ...
        toneClassNames[tone],
        isBackground && 'text-[rgba(63,53,44,0.82)]',
        compact
        ...
        selected
          ? isBackground
            ? 'border-[rgba(107,95,80,0.2)] ring-1 ring-[rgba(107,95,80,0.08)]'
      ...
      <div className="flex items-center justify-between gap-3">
        <div
          className={clsx(
            ...
            isBackground && 'bg-[rgba(32,23,15,0.045)] text-[rgba(104,92,78,0.92)]',
          )}
        >
      ...
      <div className={clsx(compact ? 'mt-2' : 'mt-4')}>
        ...
        {compact ? null : (
          <p
            className={clsx(
              'text-[var(--muted)]',
              supporting ? 'mt-2 text-[13px] leading-5' : 'mt-3 text-sm leading-6',
              isBackground && 'text-[rgba(111,97,82,0.9)]',
            )}
          >
            {card.body}
          </p>
